...
public class KVServer {
    ...
    private void load(HttpExchange h) throws IOException{
        try {
            ...
            if ("GET".equals(h.getRequestMethod())) {
                ...
                if (key.isEmpty()) {
                    System.out.println("Key для заргузки пустой. key указывается в пути: /load/{key}");
                    h.sendResponseHeaders(400, 0);
                    return;
                }

                if (!data.containsKey(key)) {
                    System.out.println("Такого ключа key = " + key + " не существует.");
    ...
    public void stop() {
        System.out.println("Останавливаем сервер KVServer.");
        server.stop(0);
    }
}